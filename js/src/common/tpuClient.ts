import { Commitment, ConfirmOptions, Connection, ConnectionConfig, PublicKey, SendOptions, Signer, Transaction, TransactionConfirmationStrategy, TransactionSignature, VersionedTransaction } from "@solana/web3.js";
...
export class TpuConnection extends Connection {
     ...
     * @returns {Promise<string>}
     */
    // sendTransaction(transaction: Transaction | VersionedTransaction, signers: Array<Signer>, options?: SendOptions): Promise<string> {
    //     return this.tpuClient.sendTransaction(transaction, signers, options);
    //     new Connection("").sendTransaction
    // }
     ...
     */
    async sendAndConfirmTransaction(connection: TpuConnection, transaction: Transaction, signers: Array<Signer>, recentBlockhash: Readonly<{ blockhash: string; lastValidBlockHeight: number; }>, commitment?: Commitment) : Promise<TransactionSignature> {
        const signature = await this.tpuClient.sendTransaction(transaction, signers);
     ...
     * @returns {Promise<string>}
     */
    async sendAndConfirmRawTransaction(connection: TpuConnection, rawTransaction: Buffer | Array<number> | Uint8Array, recentBlockhash: Readonly<{ blockhash: string; lastValidBlockHeight: number; }>, commitment?: Commitment) : Promise<string> {
